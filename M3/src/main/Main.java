...
public class Main {
	public static void main(String[] args) {
		...
		System.out.println("se inserta juan12 en la BBDD");
		sc.nextLine();
		...
		while (true) {
			try {
				...
				usuario.setWeapon(weaponContainer.get(6));
				break;
			} catch (Errores e1) {
				e1.printStackTrace();
		...
		System.out.println("comprobar juan10");
		sc.nextLine();
		
		
		dBBD.insertarBattleInicio(enemigo1, usuario);
		System.out.println("Se inserto en BATTLE la info");
		sc.nextLine();
		while (true) {
		...
		dBBD.borrarUsuarioDeBattle(usuario);
